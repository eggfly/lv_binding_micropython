...
void panel_device_reset(panel_sh8601 * panel)
{
  int16_t pin = panel->_cfg.pin_rst;
  if (pin < 0) return;
  gpio_hi(pin);
  delay(64);
  gpio_lo(pin);
...
void panel_device_init_rst(panel_sh8601 * panel)
{
  int16_t pin = panel->_cfg.pin_rst;
  if (pin < 0) return;
  // lgfx::gpio_hi(pin);
...
void panel_device_init(panel_sh8601 * panel)
{
  panel_device_init_cs(panel);
  qspi_init(&panel->_bus);
  // panel->_bus->init();
  panel_device_init_rst(panel);
  if (use_reset)
  {
    panel_device_reset(panel);
  }
  return true;
}
...
/* Panel init */
bool panel_sh8601_init(panel_sh8601 * panel, bool use_reset)
{
  ESP_LOGD("SH8601Z", "panel init %d", use_reset);

  if (!panel_device_init(use_reset))
  {
    return false;
  }

  /* Store pannel resolution */
  _width = panel->_cfg.panel_width;
  _height = panel->_cfg.panel_height;
...
void panel_sh8601_setBrightness(panel_sh8601 * panel, uint8_t brightness)
{
...
color_depth_t Panel_SH8601Z::setColorDepth(panel_sh8601 * panel, color_depth_t depth)
{
...
void Panel_SH8601Z::write_cmd(panel_sh8601 * panel, uint8_t cmd)
{
...
void Panel_SH8601Z::start_qspi(panel_sh8601 * panel)
{
  /* Begin QSPI */
  cs_control(false);
...
void Panel_SH8601Z::end_qspi(panel_sh8601 * panel)
{
  /* Stop QSPI */
  _bus->writeCommand(0x32, 8);
  _bus->writeCommand(0x00, 8);
...
void Panel_SH8601Z::beginTransaction(panel_sh8601 * panel)
{
...
void Panel_SH8601Z::endTransaction(panel_sh8601 * panel)
{
...
void Panel_SH8601Z::setWindow(panel_sh8601 * panel, uint_fast16_t xs, uint_fast16_t ys, uint_fast16_t xe, uint_fast16_t ye)
{
...
void Panel_SH8601Z::writePixels(panel_sh8601 * panel, pixelcopy_t *param, uint32_t len, bool use_dma)
{
...
void Panel_SH8601Z::writeImage(panel_sh8601 * panel, uint_fast16_t x, uint_fast16_t y, uint_fast16_t w, uint_fast16_t h, pixelcopy_t *param, bool use_dma)
{
