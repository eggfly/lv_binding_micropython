...
// eggfly added
  ...
  inline void exec_spi(void) { *_spi_cmd_reg = SPI_EXECUTE; }
  inline void wait_spi(void)
  {
    while (*_spi_cmd_reg & SPI_USR)
      ;
  }
  inline void set_write_len(uint32_t bitlen) { *_spi_mosi_dlen_reg = bitlen - 1; }
  inline void set_read_len(uint32_t bitlen) { *reg(SPI_MISO_DLEN_REG(_spi_port)) = bitlen - 1; }
